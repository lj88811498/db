...
import com.youedata.nncloud.core.base.tips.Tip;
...
import com.youedata.nncloud.core.common.constant.dictmap.UserDict;
...
import com.youedata.nncloud.core.common.exception.BizExceptionEnum;
import com.youedata.nncloud.core.datascope.DataScope;
import com.youedata.nncloud.core.db.Db;
import com.youedata.nncloud.core.exception.GunsException;
...
import com.youedata.nncloud.core.util.ImgDirUtil;
...
import com.youedata.nncloud.modular.system.factory.UserFactory;
import com.youedata.nncloud.modular.system.model.User;
import com.youedata.nncloud.modular.system.service.IUserService;
import com.youedata.nncloud.modular.system.transfer.UserDto;
import com.youedata.nncloud.modular.system.warpper.UserWarpper;
...
public class UserMgrController extends BaseController {
    ...
    public Tip edit(@Valid UserDto user, BindingResult result) throws NoPermissionException {
        if (user.getAvatar() != null) {
            String img = user.getAvatar().split("fx")[1];
            user.setAvatar(img); // 设置头像路径
        }
        if (result.hasErrors()) {
            throw new GunsException(BizExceptionEnum.REQUEST_NULL);
        }
    ...
    //@ApiOperation("上传图片")
    @ResponseBody
    public Object upload(@RequestParam("file") MultipartFile picture) {
        JSONObject js = JsonUtil.createOkJson();
        //InputStream inputStream = picture.getInputStream();
        String pictureName = UUID.randomUUID().toString() + ".jpg";
//        String imgOrgName = picture.getOriginalFilename(); // 图片名
//        String imgPath = ImgDirUtil.getImgDir() + pictureName; // 路径
//        String imgSize = picture.getSize() / 1000 + "KB"; // 大小
        String fileSavePath = "";
        String imgDir = ImgDirUtil.getImgDir();
        try {
            // 是否为头像上传，路径不一样
            fileSavePath = gunsProperties.getFileUploadPath() + imgDir;
            File file = new File(fileSavePath);
            if (!(file.exists() && file.isDirectory())) {
                file.mkdirs();
            }
            picture.transferTo(new File(fileSavePath + pictureName));
            return JsonUtil.createOkJson(gunsProperties.getFileUploadPath() + imgDir + pictureName);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
